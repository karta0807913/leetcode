...
func (h NodeHeap) Less(i, j int) bool {
	// pop from smaller to bigger
	return h[i].DelayTime < h[j].DelayTime
}
...
func networkDelayTime(times [][]int, n int, k int) int {
	var nodeList NodeHeap = make([]*Node, n)
	...
	nodeList[k-1].DelayTime = 0
	for _, time := range times {
		if time[1] == k {
			continue
		}
		nodeList[time[0]-1].Edges = append(nodeList[time[0]-1].Edges, Edge{
			Delay: time[2],
			Next:  nodeList[time[1]-1],
		})
	}
	heap.Init(&nodeList)
	...
	max := 0
	for nodeList.Len() != 0 {
		node := heap.Pop(&nodeList).(*Node)
		max = node.DelayTime
		if node.DelayTime == math.MaxInt64 {
			return -1
		}
		for idx := range node.Edges {
			delay := node.Edges[idx].Delay + node.DelayTime
			next := node.Edges[idx].Next
			if next.DelayTime > delay {
				next.DelayTime = delay
				nodeList.Fix(next)
			}
		}
	}
...
func TestDelayTime(t *testing.T) {
	assert.Equal(t, 34, networkDelayTime([][]int{
	...
	assert.Equal(t, 59, networkDelayTime([][]int{
	...
	assert.Equal(t, 2, networkDelayTime([][]int{
		{2, 1, 1}, {2, 3, 1}, {3, 4, 1},
	}, 4, 2))
	assert.Equal(t, 2, networkDelayTime([][]int{
		{2, 3, 1}, {3, 4, 120}, {1, 4, 1}, {2, 1, 1},
	}, 4, 2))
	assert.Equal(t, 2, networkDelayTime([][]int{
		{2, 3, 1}, {3, 4, 120}, {1, 4, 1}, {2, 1, 1}, {4, 1, 1}, {1, 2, 3},
	}, 4, 2))
	assert.Equal(t, -1, networkDelayTime([][]int{
		{2, 3, 1}, {3, 4, 120}, {1, 4, 1}, {2, 1, 1}, {4, 1, 1}, {1, 2, 3},
	}, 5, 2))
	assert.Equal(t, 999, networkDelayTime([][]int{
		{2, 3, 1}, {3, 4, 120}, {1, 4, 1}, {2, 1, 1}, {4, 1, 1}, {1, 2, 3}, {2, 5, 999},
	}, 5, 2))
	assert.Equal(t, 1, networkDelayTime([][]int{
		{1, 2, 1},
	}, 2, 1))
